...
class HDF5Source(DepthSource):
    ...
    def __init__(self, file, dataset_index=0):
        self.file = file

        self.info = self.file['inf' + str(dataset_index)]
        self.timestamps = self.file['tim' + str(dataset_index)]
        self.images = self.file['img' + str(dataset_index)]
        self.depth_maps = self.file['dep' + str(dataset_index)]
    ...
    def about(self):
        return f"hdf5 depth source using {self.filename}"
